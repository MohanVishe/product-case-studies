...
- **In the experiment**, the 3B model would have to be **3.84× cheaper per token** than the 7B just to break even per successful task at the orchestrator, where it passed 22% of tasks against 63%. At a leaf node in the same system, break-even was **1.13×**, so there the price gap passes almost straight through.
...
None of this is expensive. The evaluation below, 240 agent conversations and nearly 300 router calls, ran on one consumer GPU in 56 minutes, at no cost.
...
Each model ran 120 conversations (24 tasks × 5) and 144 router calls (48 messages × 3).
...
watching call counts, the downgrade would look free.
...
| Lookups / multi-hop / writes / multi-turn | 53% / 0% / 27% / 7% | 87% / 47% / 80% / 40% |
...
**The break-even price ratio at the orchestrator is 3.84×.** The 3B would have to be nearly four
times cheaper per token than the 7B before the swap paid for itself, and that's only the cost
argument. The quality gate settles it first: 22% against 63%, and 4% against 50% when the same
conversation has to work five times out of five.
...
holds up 4% of the time. The 7B is not brilliant either, at 50%, which is roughly what τ-bench
found for much larger models on comparable tasks. It's a reminder of how much distance there is
between "it worked when I tried it" and "it works".

### The same cheap model, at a leaf node

| Intent router | 3B | 7B |
|---|---:|---:|
| Accuracy over 144 calls | 87% | 98% |
| Input tokens per call | 118 | 118 |
| Tokens per correct call | 149 | 132 |
| Latency per call | 2.25 s | 2.37 s |
| **Break-even price ratio** | **1.13×** | |

One call, a fixed output, no loop, no accumulating context. Both models send the same 118 tokens,
and the break-even ratio collapses from 3.84× to **1.13×**: almost the whole price gap passes
...
**Two nodes, one system, the same two models, opposite answers.** That is the whole argument of
this piece, measured rather than asserted.
...
- Token counts are uncached. With prompt caching the cost gap narrows, as Case 3 above shows,
  though the quality and reliability gaps don't move.
- Latency is local, on one consumer GPU, with nothing else running. Hosted APIs behave differently,
  and a hosted small model would usually be faster per call than this one was.
- The graders check end state and required facts, not tone, and the multi-turn users are scripted,
  which is easier than real ones.
- 24 tasks is a starting evaluation set, not a benchmark. The point is that a set this small,
  running in under an hour at no cost, already answered the question.
